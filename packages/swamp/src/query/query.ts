...
import { createS3SecretStatement, generateRandomAlphanumeric, sleep, uploadFileToS3 } from '../util';
import { ColumnSchema, ColumnsSchema, CreateRecordsEvent, DataLocation, LoaderInserts, RawLoaderInserts, S3Credentials } from '../core/types';
import fs  from 'fs';
...
function escapeColumn(column: string): string {
  return DUCKDB_KEYWORDS.includes(column.toLowerCase()) ? `"${column}"` : column;
}

function generateCreateTableStatement(tableName: string, schema: Record<string, ColumnSchema>): string {
  const columns = Object.entries(schema).map(([_columnName, columnSchema]) => {
    const innerColumnType = columnSchema.columnType;
    const columnType = innerColumnType.type === 'ARRAY' ? `${innerColumnType.innerType}[]` : innerColumnType.type;
  ...
  const columnsDefinition = columns.join(', ') + ', _swamp_updated_at TIMESTAMP';
  return `CREATE TABLE ${tableName} (${columnsDefinition})`;
}

type CreateNewTableResult = {
  filePath: string;
  recordCount: number;
};

async function createNewTable(db: Database, records: Record<string, any>[], schema: Record<string, ColumnSchema>, writePath: string): Promise<Result<CreateNewTableResult, string>> {
  ...
  for (const record of records) {
    ...
    try {
      const tryInsert = await all(db, `insert into ${tempTable} (${columns.join(', ')}) values (${values.map(() => '?').join(', ')})`, values);
      if (tryInsert.isErr()) {
...
async function mergeSchemas(existingSchema: ColumnsSchema, newSchema: ColumnsSchema): Promise<ColumnsSchema> {
  // todo: handle case where column type changes
  const onlyInNew: ColumnsSchema = Object.fromEntries(Object.entries(newSchema).filter(([column]) => existingSchema[column] === undefined));
  ...
  async writeRecords(inserts: LoaderInserts): Promise<void> {
    ...
    try {
      ...
      for (const [tableName, batch] of Object.entries(inserts)) {
        ...
        const writeFolder = existingTable === null ? this.getLocalWriteFolder(schemaName, tableName) : existingTable.location;
        fs.mkdirSync(writeFolder, { recursive: true });

        const fileName = this.getFileName();
        const writePath = `${writeFolder}/${fileName}`;

        const parquetFile = await createNewTable(db, records, columnSchema, writePath);
        if (parquetFile.isErr()) {
          console.error(`Error writing table ${schemaName}.${tableName}: ${parquetFile.error}`);
          console.log('records', records);
          continue;
        }
        console.log(`Wrote ${parquetFile.value.recordCount} records to table ${schemaName}.${tableName}`);

        const newColumnSchema = existingTable === null ? columnSchema : await mergeSchemas(existingTable.columnSchema, columnSchema);
