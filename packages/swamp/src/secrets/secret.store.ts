...
export abstract class SecretStore {
  abstract get<T extends z.ZodObject<any>>(uniqueId: string, secretSchema: T): Result<z.infer<T>, string>;
...
export class EnvVarSecretStore extends SecretStore {
  ...
  get<T extends z.ZodObject<any>>(uniqueId: string, secretSchema: T): Result<z.infer<T>, string> {
    const secretKeys: [string, z.ZodString | z.ZodNumber][] = Object.entries(secretSchema.shape);
    ...
    for (const [secretKey, secretType] of secretKeys) {
      ...
      if (envVarValue) {
        ...
        console.log(`Connector ${uniqueId} missing environment variable ${envVar}`);
        return err(`Connector ${uniqueId} missing environment variable ${envVar}`);
      }
    }
