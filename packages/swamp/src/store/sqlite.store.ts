...
import { z } from 'zod';
import { ColumnSchema, S3Credentials } from '../core/types';
...
const TableObject = z.object({
  ...
  dataType: z.union([z.literal('json'), z.literal('parquet')]),
  fileLocation: z.union([z.literal('local'), z.literal('s3')]),
  columnSchema: z.record(ColumnSchema),
});
export type TableObject = z.infer<typeof TableObject>;
...
export type TableStoreInterface = {
  addTable: (tableObject: TableObject) => Promise<void>,
  getTables: (uniqueId?: string) => Promise<TableObject[]>,
  getTable: (schema: string, table: string) => Promise<TableObject | null>,
};
...
const SETUP_STATEMENTS = [CREATE_TABLE_STATEMENT, CREATE_CURSOR_STATEMENT, CREATE_RUN_STATEMENT];

function addTable(db: SQLiteDatabase, table: TableObject): void {
  // console.log('Adding table', table);
  const { tableName, schemaName, location, dataType, columnSchema, fileLocation } = table;
  db.prepare(`
    INSERT OR REPLACE INTO ${CATLOG_TABLE_NAME} (tableName, schemaName, location, dataType, columnSchema, fileLocation) 
      VALUES (?, ?, ?, ?, ?, ?)
    `,
  ).run(tableName, schemaName, location, dataType, JSON.stringify(columnSchema), fileLocation);
}

function getTable(db: SQLiteDatabase, schemaName: string, tableName: string): TableObject | null {
  const result = db.prepare<{}, InnerTableObject>(`SELECT * FROM ${CATLOG_TABLE_NAME} WHERE schemaName = ? AND tableName = ?`).get([schemaName, tableName]);
  if (result) {
    return {
      ...result,
...
function getTables(db: SQLiteDatabase, uniqueId?: string): TableObject[] {
  if (uniqueId) {
    const tables = db.prepare<{}, InnerTableObject>(`SELECT * FROM ${CATLOG_TABLE_NAME} where schemaName = ?`).all(uniqueId);
    ...
    const tables = db.prepare<{}, InnerTableObject>(`SELECT * FROM ${CATLOG_TABLE_NAME}`).all({});
    return tables.map((table) => ({
      ...table,
      columnSchema: JSON.parse(table.columnSchema),
    }));
...
function addCursor(db: SQLiteDatabase, uniqueId: string, cursor: Record<string, any>): void {
  db.prepare(`
...
function getCursor(db: SQLiteDatabase, uniqueId: string): Record<string, any> | null {
  const result = db.prepare<{}, { cursor: string }>(`SELECT cursor FROM ${CURSOR_TABLE_NAME} WHERE uniqueId = ?`).get(uniqueId);
  if (result) {
...
function getLatestRun(db: SQLiteDatabase, uniqueId: string): LoaderRunOutput | null {
  const result = db.prepare<{}, LoaderRunOutput>(`SELECT status, message, createdAt FROM ${RUN_TABLE_NAME} WHERE uniqueId = ? ORDER BY createdAt DESC LIMIT 1`).get(uniqueId);
  return result || null;
}
...
export class SQLiteStore implements Store {
  ...
  async addTable(table: TableObject): Promise<void> {
    addTable(this.db, table);
  }

  async getTable(schemaName: string, tableName: string): Promise<TableObject | null> {
    return getTable(this.db, schemaName, tableName);
  }

  async getTables(uniqueId?: string): Promise<TableObject[]> {
    return getTables(this.db, uniqueId);
  }

  async addCursor(uniqueId: string, cursor: Record<string, any>): Promise<void> {
