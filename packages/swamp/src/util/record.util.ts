...
function duckDBTypeForIntermediate(
...
function determineColumnSchema(
  ...
  Object.entries(types).forEach(([key, valueTypes]) => {
    ...
    if (valueTypes.has("string")) {
      ...
      const innerType = [...valueTypes]
        .find((type) => type.startsWith("array:"))!
        .split(":")[1] as IntermediateType;
      const innerDuckDBType = duckDBTypeForIntermediate(innerType);
      schema[key] = {
        columnType: { type: "ARRAY", innerType: innerDuckDBType },
        isPrimaryKey,
      };
...
export function processInserts(
  inserts: Inserts,
  schemaName: string
): LoaderInserts {
  const cleanedInserts: LoaderInserts = {};

  Object.entries(inserts).forEach(
    ([batchName, { records, primaryKeys: _primaryKeys }]) => {
      const allKeys = new Set<string>();
      let primaryKeys = _primaryKeys;

      // First pass to collect all keys and clean records
      const cleanedRecords = records.map((record) => {
        const firstPass: Record<string, any> = Object.fromEntries(
